...
bool ValidateEnPassant(const Move & m, const Position & p) {
    ...
    const Square::LightSquare ep = p.EnPassant();
...
bool ValidateCastling(const Move & m, const Position & p){
    ...
    switch(m.Type()){
    case(sm_Ks):
        if ( (p.WhitePlayer().CastlingRight() & Player::cr_ks) == 0){
        ...
        break;
    case(sm_Qs):
        if ( (p.WhitePlayer().CastlingRight() & Player::cr_qs) == 0){
        ...
        break;
    case(sm_ks):
        if ( (p.BlackPlayer().CastlingRight() & Player::cr_ks) == 0){
        ...
        break;
    case(sm_qs):
        if ( (p.BlackPlayer().CastlingRight() & Player::cr_qs) == 0){
    ...
    ///@todo can easily use bitboard for this
...
bool ValidateMove(const Move &m, Position &p){
    if ( ! Definitions::algo.do_trustedGenerator) {
      if (! UtilMove::ValidateCheckStatus(m,p) ) return false;
