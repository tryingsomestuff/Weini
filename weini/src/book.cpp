...
std::unordered_map<Util::Zobrist::HashType, std::set<Book::BookMove> > Book::_book;
...
void Book::AddLine(const std::string & line, bool algAbr, bool trusted, std::ofstream * binFile){
   ...
   std::string moveStr;
   static Position ps(Position::startPosition);
   Position p(ps);
   int k = 0;
   while( str >> moveStr ){
       ++k;
...
bool Book::Add(const std::string & moveAlg, Position & p, bool algAbr, bool trusted, std::ofstream * binFile){
   ...
   if (trusted) {
       p.ApplyMove(m, 0, true);
   }
   else {
       p = m.ValidateAndApply(p, &ok);
   }
   if (!ok) {
...
void Book::ReadBinaryBook(std::ifstream & stream, bool trusted) {
    LOG(logINFO) << "Book is in binary format.";
    static Position ps(Position::startPosition);
    Position p(ps);
    ...
    while (!stream.eof()) {
        ...
        bool ok = true;
        if (trusted) {
            UtilMove::DetectStandardCapture(m,p);
            if ( UtilMove::PossibleEnPassant(m,p) && !UtilMove::ValidateEnPassant(m,p)){
                LOG(logFATAL) << "Book move lead seems to be not a legal en-passant in this position " << p.GetFEN() << ", move was " << m.Show();
            }
            // in case Book move was tagged ep but is not en-passant in the current position
            if ( m.Type() == UtilMove::sm_ep && !m.IsCapture() ){
                LOG(logFATAL) << "Book move tagged en-passant, but not valid in this position " << p.GetFEN() << ", move was " << m.Show();
            }
            p.ApplyMove(m, 0, true);
        }
        else {
    ...
    LOG(logINFO) << count << " lines read";
}

bool Book::ReadBook(bool trusted){

    LOG(logINFO) << "Loading books (async)";
    ...
    if (Definitions::bookConfig.with_bigBook) {
        ...
        if (!isBinary) {
            ...
            if (bookFile) {
                ...
                std::string line;
                static std::mt19937_64 mt(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now().time_since_epoch()).count());
                static auto randDouble = std::uniform_real_distribution<double>(0,1);
                size_t randStart = size_t(Definitions::bookConfig.bookSkip * randDouble(mt));
                while (std::getline(bookFile, line)) {
                    ++l;
                    if (l < randStart) continue;
                    if (l % 10000 == 0) {
                        int p = int(100.*l / lines);
                        LOG(logINFO) << p << "%" << " (" << count+1 << "/" << lines / Definitions::bookConfig.bookSkip << ")";
                    }
            ...
            ReadBinaryBook(bookFileBin,!Definitions::bookConfig.debugBook);
            bookFileBin.close();
        }
    }
...
const Move *Book::Get(Util::Zobrist::HashType h){
   std::unordered_map<Util::Zobrist::HashType,std::set<BookMove> >::iterator it = _book.find(h);
