...
void TestGameZHash(){
  ...
  */
  ...
  Move("d5 d4", p.Turn()).ApplyInPlace(p).Display();

  // test capture with pawn
  Move("e4 e5", p.Turn()).ApplyInPlace(p).Display();

  GETHASH;

  // bishop forward
  Move("c8 d7", p.Turn()).ApplyInPlace(p).Display();

  // bishop forward
  Move("f1 e2", p.Turn()).ApplyInPlace(p).Display();

  // bishop backward
  Move("d7 c8", p.Turn()).ApplyInPlace(p).Display();

  // bishop backward
  Move("e2 f1", p.Turn()).ApplyInPlace(p).Display();

  COMPAREHASH;

  p = Position("r3kbnr/pppqpppp/2npb3/8/8/2NBPN2/PPPP1PPP/R1BQK2R w KQkq - 6 5");

  Move("0-0", p.Turn()).ApplyInPlace(p).Display();
  Move("0-0-0", p.Turn()).ApplyInPlace(p).Display();
...
std::deque<SearcherBase::SearchedMove> Play(const std::string & fen, DepthType depth, bool AIVsAI){
    ...
    while(true){
       ...
       // is it a draw ?
       if ( Searcher::IsForcedDraw(gamehash,p,3,true) ){
