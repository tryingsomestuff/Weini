    ...
    for(unsigned char bid = 0 ; bid < 2 ; ++bid){
        ...
        if ( tt.depth >= mindepth){
            ...
            case Transposition::tt_exact:
                ...
                if ( std::abs(tt.score) > Definitions::scores.checkMateScore - MAX_GAME_PLY ) return false;
                /*
                if ( tt.score > Definitions::scores.checkMateScore - MAX_GAME_PLY ){
                    tt.score = Definitions::scores.checkMateScore-currentPly-tt.depth;
                }
                else if ( tt.score < -Definitions::scores.checkMateScore + MAX_GAME_PLY ){
                    tt.score = -Definitions::scores.checkMateScore+currentPly+tt.depth;
                }
                */
                return true;
...
std::string Transposition::GetPV(const Position & pp, int depth, bool abr){
    ...
    if (count == 0) {
        LOG(logERROR) << "Sorry first move in PV is not in the TT anymore";
    }
