...
void Definitions::PieceValue::ChangeHook(){
...
Definitions::TimeControl::TimeControl() {
    currentMoveMs               = 10000;     // msec
    timeControl                 = 120;       // msec
    increment                   = 0;         // msec
    movePerSession              = 40;        // number of move until next TC start
    emergencyTimeFactor         = 5;
...
Definitions::Selectivity::Selectivity(){
    ...
    iterativeDeepeningSelectivityMinDepth = 2;
...
Definitions::TranspositionTableConfig::TranspositionTableConfig(){
    do_transpositionTableSearch        = false; // this losses elo (a lot)
    ...
    allTtSize           = 0; // manual mode is default
    ttSize              = 72;
    ttQSize             = 36;
    ttESize             = 64;
    ttELSize            = 32;
...
namespace{
void CheckOnlyOnceTT(bool & onlyOnce){
    if ( ! onlyOnce ){
        onlyOnce = true;
    }
    else{
        LOG(logERROR) << "TT size cannot be changed !!";
    }
}
}

void Definitions::TranspositionTableConfig::ChangeHookTTAll(){
    static bool onlyOnce = false;
    CheckOnlyOnceTT(onlyOnce);
}

void Definitions::TranspositionTableConfig::ChangeHookTT(){
    static bool onlyOnce = false;
    CheckOnlyOnceTT(onlyOnce);
}

void Definitions::TranspositionTableConfig::ChangeHookTTQ(){
    static bool onlyOnce = false;
    CheckOnlyOnceTT(onlyOnce);
}

void Definitions::TranspositionTableConfig::ChangeHookTTE(){
    static bool onlyOnce = false;
    CheckOnlyOnceTT(onlyOnce);
}

void Definitions::TranspositionTableConfig::ChangeHookTTEL(){
    static bool onlyOnce = false;
    CheckOnlyOnceTT(onlyOnce);
}

void Definitions::TranspositionTableConfig::ChangeHookTTEP(){
    static bool onlyOnce = false;
    CheckOnlyOnceTT(onlyOnce);
}
...
Definitions::ExtensionReductionConfig::ExtensionReductionConfig(){
    ...
    do_recursiveReduction              = true;
    do_recursiveExtension              = false;
...
Definitions::Algorithms::Algorithms(){
    ...
    do_lmrQuiesce         = false;  ///@todo !? don't know what this could be ...
    do_lmrSearch          = true;
...
void Definitions::InitKeys() {
    ...
    // Piece values
    //*****************************
    _keys.push_back(KeyBase(k_score, "pawnValue"      , &pieceValue.pawnValueW    , std::bind(&PieceValue::ChangeHook, &pieceValue)));
    _keys.push_back(KeyBase(k_score, "knightValue"    , &pieceValue.knightValueW  , std::bind(&PieceValue::ChangeHook, &pieceValue)));
    _keys.push_back(KeyBase(k_score, "bishopValue"    , &pieceValue.bishopValueW  , std::bind(&PieceValue::ChangeHook, &pieceValue)));
    _keys.push_back(KeyBase(k_score, "rookValue"      , &pieceValue.rookValueW    , std::bind(&PieceValue::ChangeHook, &pieceValue)));
    _keys.push_back(KeyBase(k_score, "queenValue"     , &pieceValue.queenValueW   , std::bind(&PieceValue::ChangeHook, &pieceValue)));
    _keys.push_back(KeyBase(k_score, "kingValue"      , &pieceValue.kingValueW    , std::bind(&PieceValue::ChangeHook, &pieceValue)));
    _keys.push_back(KeyBase(k_score, "pawnEGValue"    , &pieceValue.pawnEGValueW  , std::bind(&PieceValue::ChangeHook, &pieceValue)));
    _keys.push_back(KeyBase(k_score, "knightEGValue"  , &pieceValue.knightEGValueW, std::bind(&PieceValue::ChangeHook, &pieceValue)));
    _keys.push_back(KeyBase(k_score, "bishopEGValue"  , &pieceValue.bishopEGValueW, std::bind(&PieceValue::ChangeHook, &pieceValue)));
    _keys.push_back(KeyBase(k_score, "rookEGValue"    , &pieceValue.rookEGValueW  , std::bind(&PieceValue::ChangeHook, &pieceValue)));
    _keys.push_back(KeyBase(k_score, "queenEGValue"   , &pieceValue.queenEGValueW , std::bind(&PieceValue::ChangeHook, &pieceValue)));
    _keys.push_back(KeyBase(k_score, "kingEGValue"    , &pieceValue.kingEGValueW  , std::bind(&PieceValue::ChangeHook, &pieceValue)));
    ...
    _keys.push_back(KeyBase(k_bool, "do_ttClearing"                      , &ttConfig.do_ttClearing));

    _keys.push_back(KeyBase(k_ull,  "allTtSize"                          , &ttConfig.allTtSize , std::bind(&TranspositionTableConfig::ChangeHookTTAll, &ttConfig)));
    _keys.push_back(KeyBase(k_ull,  "ttSize"                             , &ttConfig.ttSize    , std::bind(&TranspositionTableConfig::ChangeHookTT   , &ttConfig)));
    _keys.push_back(KeyBase(k_ull,  "ttQSize"                            , &ttConfig.ttQSize   , std::bind(&TranspositionTableConfig::ChangeHookTTQ  , &ttConfig)));
    _keys.push_back(KeyBase(k_ull,  "ttESize"                            , &ttConfig.ttESize   , std::bind(&TranspositionTableConfig::ChangeHookTTE  , &ttConfig)));
    _keys.push_back(KeyBase(k_ull,  "ttELSize"                           , &ttConfig.ttELSize  , std::bind(&TranspositionTableConfig::ChangeHookTTEL , &ttConfig)));
    _keys.push_back(KeyBase(k_ull,  "ttEPSize"                           , &ttConfig.ttEPSize  , std::bind(&TranspositionTableConfig::ChangeHookTTEP , &ttConfig)));
    ...
    _keys.push_back(KeyBase(k_bool, "do_recursiveExtension"              , &extensionReductionConfig.do_recursiveExtension));
    _keys.push_back(KeyBase(k_bool, "do_recursiveReduction"              , &extensionReductionConfig.do_recursiveReduction));
...
template<>
bool & Definitions::GetValue<bool>(const std::string & key) {
    KeyBase & k = GetKey(key);
    if (k.type != k_bool) {
        LOG(logERROR) << "Bad type for bool";
    }
    return *static_cast<bool*>(k.value);
}

template<>
ScoreType & Definitions::GetValue<ScoreType>(const std::string & key) {
    KeyBase & k = GetKey(key);
    if (k.type != k_score) {
        LOG(logERROR) << "Bad type for score";
    }
    return *static_cast<ScoreType*>(k.value);
}

template<>
DepthType & Definitions::GetValue<DepthType>(const std::string & key) {
    KeyBase & k = GetKey(key);
    if (k.type != k_depth) {
        LOG(logERROR) << "Bad type for depth";
    }
    return *static_cast<DepthType*>(k.value);
}

template<>
float & Definitions::GetValue<float>(const std::string & key) {
    KeyBase & k = GetKey(key);
    if (k.type != k_float) {
        LOG(logERROR) << "Bad type for float";
    }
    return *static_cast<float*>(k.value);
}

template<>
unsigned long long & Definitions::GetValue<unsigned long long>(const std::string & key) {
    KeyBase & k = GetKey(key);
    if (k.type != k_int) {
        LOG(logERROR) << "Bad type for int";
    }
    return *static_cast<unsigned long long*>(k.value);
}

///@todo string and vectors ...
