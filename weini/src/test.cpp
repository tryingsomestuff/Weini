...
namespace{
std::vector<std::string> Split(const std::string & line, char sep, char delim){
...
bool Test(const std::string & testString){

    std::vector<std::string> options = Split(testString,' ', ':');

    LOG(logINFO) << "test options ";
    for (auto it = options.begin() ; it != options.end() ; ++it){
        LOG(logINFO) << "             " << *it;
    }

    if ( options.empty()){
        LOG(logERROR) << "No test options given";
        return false;
    }

    if (options[0] == "help") {
        LOG(logINFO) << "Available analysis tests";
        LOG(logINFO) << " BT2630";
        LOG(logINFO) << " arasan";
        LOG(logINFO) << " arasan_sym";
        LOG(logINFO) << " CCROHT";
        LOG(logINFO) << " BKTest";
        LOG(logINFO) << " EETest";
        LOG(logINFO) << " KMTest";
        ...
        LOG(logINFO) << " sbdTest";
        LOG(logINFO) << " STS";
        LOG(logINFO) << "Available unitary tests";
        LOG(logINFO) << " xortest zhash_1 zhash_2 ... zhash_n : test zobrist hashing";
        LOG(logINFO) << " gameplay : very old test that forces a game";
        LOG(logINFO) << " zhash : another hash test";
        LOG(logINFO) << " movegenerator : test move generator";
        LOG(logINFO) << " threats : test threat detection";
        LOG(logINFO) << " see : test static exchange evaluation";
        LOG(logINFO) << " ordering fen : test move ordering";
        LOG(logINFO) << " generator fen depth display : test move generator";
        LOG(logINFO) << " divide (not working yet)";
        LOG(logINFO) << " staticanalysis fen : test static evaluation of a position";
        LOG(logINFO) << " analysis fen depth display : test analysis of a position";
        LOG(logINFO) << " playUI fen depth : console mode game against the UI";
        LOG(logINFO) << " playAI fen depth : console mode test match AI vs AI";
        LOG(logINFO) << " texel_tuning inputfile : ru texel tuning based on given file";
        return 0;
    }
    ...
    if (options[0] == "arasan") {
       ...
       ExtendedPosition::Test(positions,timeControls,true,scores,[](int score){return score;},false,true);
       return true;
    }

    if (options[0] == "arasan_sym") {
        std::vector<std::string> positions;
    ...
    // Test XOR of hashes
    //**********************************************************************
    if (options[0] == "xortest") {
        std::vector<Util::Zobrist::HashType> hashes;
        for (int k = 1; k < options.size(); ++k) {
            std::stringstream str;
            str << options[k];
            Util::Zobrist::HashType h = 0;
            str >> h;
            LOG(logINFO) << "Read hash " << h;
            hashes.push_back(h);
        }
        Util::Zobrist::HashType h = hashes[0];
        LOG(logINFO) << "hash " << h;
    ...
    //**********************************************************************
    // Play a game with valid and invalid moves, and evaluate each position
    //**********************************************************************
    if ( options[0] == "gameplay"){
     TestGamePlay();
    }
    ...
    // Test move generator
    //**********************************************************************
    if ( options[0] == "movegenerator"){
       TestGenerator(Position::startPosition,{20,400,8902,197281,4865609,119060324,3195901860,84998978956},4);
       TestGenerator("r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 0",{48,2039,97862,4085603},3);
       TestGenerator("8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 0",{14,191,2812,43238,674624},4);
       TestGenerator("r3k2r/Pppp1ppp/1b3nbN/nP6/BBP1P3/q4N2/Pp1P2PP/R2Q1RK1 w kq - 0 1",{6,264,9467,422333},3);
       TestGenerator("rnbq1k1r/pp1Pbppp/2p5/8/2B5/8/PPP1NnPP/RNBQK2R w KQ - 1 8",{44,1486,62379,2103487},3);
       TestGenerator("r4rk1/1pp1qppp/p1np1n2/2b1p1B1/2B1P1b1/P1NP1N2/1PP1QPPP/R4RK1 w - - 0 10",{46,2079,89890,3894594},3);
       TestGenerator("rnbqkbnr/8/8/8/8/8/8/RNBQKBNR w KQkq - 0 1",{50,2125},1);
       TestGenerator("4k3/pppppppp/8/8/8/8/PPPPPPPP/4K3 w - - 0 1",{18,324},1);
       TestGenerator("n1n5/PPPk4/8/8/8/8/4Kppp/5N1N b - - 0 1",{24,496,9483},2);
    }

    //**********************************************************************
    // Test for threats
    //**********************************************************************
    if ( options[0] == "threats"){
       TestThreats(Position::startPosition,"a1",{});
       TestThreats(Position::startPosition,"d1",{});
       TestThreats(Position::startPosition,"d5",{});
       TestThreats(Position::startPosition,"b3",{});
       TestThreats("8/8/3k4/6B1/3b1n1r/4P1p1/R6p/5NQK b - -","h2",{Square("h1"),Square("g1"),Square("f1"),Square("a2")});
       TestThreats("8/8/3k4/6B1/3b1n1r/4P1p1/R6p/5NQK b - -","g8",{});
       TestThreats("8/8/3k4/6B1/3b1n1r/4P1p1/R6p/5NQK b - -","f4",{Square("e3"),Square("g5")});
       TestThreats("8/8/3k4/6B1/3b1n1r/4P1p1/R6p/5NQK b - -","d4",{Square("e3")});
       TestThreats("8/8/3k4/6B1/3b1n1r/4P1p1/R6p/5NQK w - -","g1",{Square("h2")});
       TestThreats("2k5/2b5/8/8/8/8/2R5/2K5 b - - 0 1","c7",{Square("c2")});
       TestThreats("3rk2r/ppp1pPbp/2n2np1/1q3b2/8/NP3N2/PB1PQPPP/2R1KB1R w KQkq -","e2",{Square("b5")});
       TestThreats("3rk2r/ppp1pPbp/2n2np1/1q3b2/8/NP3N2/PB1PQPPP/2R1KB1R b KQkq -","c6",{Square("c1")});
       TestThreats("3rk2r/ppp1pPbp/2n2np1/1q3b2/8/NP3N2/PB1PQPPP/2R1KB1R b KQkq -","b5",{Square("a3"),Square("e2"),Square("f1")});
       TestThreats("3rk2r/ppp1pPbp/2n2np1/1q3b2/8/NP3N2/PB1PQPPP/2R1KB1R b KQkq -","f5",{});
       TestThreats("3rk2r/ppp1pPbp/2n2np1/1q3b2/8/NP3N2/PB1PQPPP/2R1KB1R w KQkq -","d2",{Square("d8")});
       TestThreats("3rk2r/ppp1pPbp/2n2np1/1q3b2/8/NP3N2/PB1PQPPP/2R1KB1R b KQkq -","e8",{Square("f7")});
       TestThreats("3rk2r/ppp1pPbp/2n2np1/1q3b2/8/NP3N2/PB1PQPPP/2R1KB1R b KQkq -","f6",{Square("b2")});
    }
    ...
    //**********************************************************************
    if ( options[0] == "see"){
       TestSEE("1q1k4/8/3b4/8/8/8/1Q6/K7 b - -","b2",0); // queen exchange
       TestSEE("1q1k4/8/3b4/8/3b4/8/1Q6/K7 b - -","b2",0); // same but cannot capture back
       TestSEE("3k4/1q6/3b4/8/8/8/1Q6/K7 w - -","b7",0);   // white to play, capture queen that just takes a rook is good :)
       TestSEE("3k4/1q3r2/8/3b4/8/1R6/1Q6/KR6 w - -","b7",0);   // white to play, // -170 (bishop vs rook)
       TestSEE("k7/r5q1/8/8/8/8/RQ6/K7 w - -","g7",0); // white just loos a knigth, but black rook is pinned, so queen is free : +580
       TestSEE("k7/r5q1/8/8/8/8/RQ6/K7 b - -","b2",0); // black just loos a knigth, but white rook is pinned, so queen takes queen and king take queen, +320
    }

    //**********************************************************************
    // Test move ordering
    //**********************************************************************
    if ( options[0] == "ordering"){
        std::string fen = Position::startPosition;
        if ( options.size() > 1 && !(options[1]=="help") ){
            fen = options[1];
        }
        else{
            LOG(logWARNING) << "ordering test needs an extra argument (fen position)";
            LOG(logINFO) << "fen string shall be inputed as :rn2kb1r/pp2pppp/1qP2n2/8/6b1/1Q6/PP1PPPBP/RNB1K1NR b KQkq - 1 6:";
            return false;
        }
    ...
    // Move generator
    //**********************************************************************
    if ( options[0] == "generator"){
        std::string fen = Position::startPosition;
        if ( options.size() > 1 && !(options[1]=="help") ){
            fen = options[1];
        }
        else{
            LOG(logWARNING) << "generator test needs an extra argument (fen position), and eventually a depth, display bool, pseudo legal bool";
            LOG(logINFO) << "fen string shall be inputed as :rn2kb1r/pp2pppp/1qP2n2/8/6b1/1Q6/PP1PPPBP/RNB1K1NR b KQkq - 1 6:";
            return false;
        }
        int depth = 1;
        ...
        TestGenerator(fen,std::vector<NodeStatType>(),depth,display,pseudolegal);
    }

    //**********************************************************************
    // Move generator (with divide)
    //**********************************************************************
    if ( options[0] == "divide"){
        std::string fen = Position::startPosition;
        if ( options.size() > 1 && !(options[1]=="help") ){
            ...
            LOG(logWARNING) << "divide test needs an extra argument (fen position)";
            LOG(logINFO) << "fen string shall be inputed as :rn2kb1r/pp2pppp/1qP2n2/8/6b1/1Q6/PP1PPPBP/RNB1K1NR b KQkq - 1 6:";
            return false;
        }
        int depth = 1;
        ...
        //TestGenerator(fen,std::vector<NodeStatType>(),depth,dlevel,display,pseudolegal);
        ///@todo divide
    }
    ...
    // Static analysis
    //**********************************************************************
    if ( options[0] == "staticanalysis"){
        std::string fen = "8/8/3k4/8/3b1n1r/6p1/7p/7K b - -";
        if ( options.size() > 1 && !(options[1]=="help") ){
            fen = options[1];
        }
        else{
            LOG(logWARNING) << "staticanalysis test needs an extra argument (fen position)";
            LOG(logINFO) << "fen string shall be inputed as :rn2kb1r/pp2pppp/1qP2n2/8/6b1/1Q6/PP1PPPBP/RNB1K1NR b KQkq - 1 6:";
            return false;
        }
        LOG(logINFO) << "Static score : " << TestStaticAnalysis(fen);
    }

    //**********************************************************************
    // Texel tuning
    //**********************************************************************
    if (options[0] == "texel_tuning") {
        std::string filename;
        if (options.size() > 1 && !(options[1] == "help")) {
            filename = options[1];
        }
        else {
            LOG(logWARNING) << "texel_tuning test needs an extra argument (a file name)";
            return false;
        }

        TexelTuning(filename);
    }

    //**********************************************************************
    // Analysis
    //**********************************************************************
    if ( options[0] == "analysis"){
        std::string fen = "8/8/3k4/8/3b1n1r/6p1/7p/7K b - -";
        if ( options.size() > 1 && !(options[1]=="help") ){
            fen = options[1];
        }
        else{
            LOG(logWARNING) << "analysis test needs an extra argument (fen position), and eventually a depth";
            LOG(logINFO) << "fen string shall be inputed as :rn2kb1r/pp2pppp/1qP2n2/8/6b1/1Q6/PP1PPPBP/RNB1K1NR b KQkq - 1 6:";
            return false;
        }
        int depth = 20;
...
#ifdef WITH_STATS
        TestAnalysis(fen,depth, Searcher::eVerb_verbose);
#else
        TestAnalysis(fen,depth, Searcher::eVerb_standard);
#endif
    }

    //**********************************************************************
    // Play against AI, command line mode
    //**********************************************************************
    if ( options[0] == "playUI"){
        std::string fen = Position::startPosition;
        if ( options.size() > 1 && !(options[1]=="help") ){
            ...
            LOG(logWARNING) << "playUI test needs an extra argument (fen position), and eventually a depth";
            LOG(logINFO) << "fen string shall be inputed as :rn2kb1r/pp2pppp/1qP2n2/8/6b1/1Q6/PP1PPPBP/RNB1K1NR b KQkq - 1 6:";
            return false;
        }
        int depth = 9;
        ...
        std::deque<SearcherBase::SearchedMove> game = Play(fen,depth,false);
        LOG(logINFO) << "Game is finished";
        Stats::CutCount();
        Util::DisplayPGN(fen,game);
    }

    //**********************************************************************
    // Play AI against AI, command line mode
    //**********************************************************************
    if ( options[0] == "playAI"){
        std::string fen = Position::startPosition;
        if ( options.size() > 1 && !(options[1]=="help")   ){
            ...
            LOG(logWARNING) << "playAI test needs an extra argument (fen position), and eventually a depth";
            LOG(logINFO) << "fen string shall be inputed as :rn2kb1r/pp2pppp/1qP2n2/8/6b1/1Q6/PP1PPPBP/RNB1K1NR b KQkq - 1 6:";
            return false;
        }
        int depth = 9;
        ...
        std::deque<SearcherBase::SearchedMove> game = Play(fen,depth,true);
        LOG(logINFO) << "Game is finished";
        Stats::CutCount();
        Util::DisplayPGN(fen,game);
    }
