...
std::string Com::DisplayInfo(DepthType depth, DepthType seldepth, ScoreType score, float sec, NodeStatType nodes, NodeStatType tbhits, const std::string & pv) {
    return protocol == e_XBoard ? XBoard::DisplayInfo(depth, seldepth, score, sec, nodes, tbhits, pv)
                                : UCI   ::DisplayInfo(depth, seldepth, score, sec, nodes, tbhits, pv);
}
...
void Com::AnalyzeUntilInput() {
    ...
    TestAnalysis(position, depth != -1 ? depth : (int)Definitions::debugConfig.defaultDepth, Searcher::eVerb_standard);
}
...
Move Com::ThinkUntilTimeUp() {
    ...
    SearcherBase::Data _data(position, depth > 0 ? depth : (int)Definitions::debugConfig.defaultDepth, Definitions::debugConfig.modedebug?Searcher::eVerb_debug:Searcher::eVerb_standard);
