...
namespace {
    void ManageExtensionRoot(DepthType & depth, const Position & previousP, const Position & currentP,
        ...
        if (Definitions::extensionConfig.do_EndGameExtensionSearch
            && previousP.IsEndGame()) {
            ++depth;
        ...
        if (Definitions::extensionConfig.do_VeryEndGameExtensionSearch
            && previousP.IsVeryEndGame()) {
            ++depth;
            ++Stats::countVeryEndGameExtensionSearch;
            if (nbExtension >= Definitions::selectivity.extensionRootMax) return;
        }
        if (Definitions::extensionConfig.do_CheckExtensionSearch
            && move.IsCheck()) {
            ++depth;
        ...
        if (Definitions::extensionConfig.do_PVExtensionSearch
            && isPV) {
            ++depth;
        ...
        if (Definitions::extensionConfig.do_SingleReplyExtensionSearch
            && moves.size() == 1) {
            ++depth;
        ...
        if (Definitions::extensionConfig.do_ReCaptureExtensionSearch
            && isPV
        ...
        if (Definitions::extensionConfig.do_CheckExtensionRoot
            && previousP.IsInCheck()) {
            ++depth;
        ...
        if (Definitions::extensionConfig.do_NearPromotionExtensionSearch
...
bool Searcher::IsForcedDraw(const Game & g, const Position & p, DepthType nbRep, bool displayDebug){
  ...
  int nbWp = p.NumberOf(Piece::t_Wpawn);
  int nbWk = p.NumberOf(Piece::t_Wknight);
  int nbWb = p.NumberOf(Piece::t_Wbishop);
  int nbWr = p.NumberOf(Piece::t_Wrook);
  int nbWq = p.NumberOf(Piece::t_Wqueen);
  int nbBp = p.NumberOf(Piece::t_Bpawn);
  int nbBk = p.NumberOf(Piece::t_Bknight);
  int nbBb = p.NumberOf(Piece::t_Bbishop);
  int nbBr = p.NumberOf(Piece::t_Brook);
  int nbBq = p.NumberOf(Piece::t_Bqueen);

  int nbW = nbWk + nbWb + nbWr + nbWq;
  int nbB = nbBk + nbBb + nbBr + nbBq;
...
ScoreType Searcher::Analyze(Position & p, bool lazy) {
    ...
    if (Searcher::IsForcedDraw(game, p, 3, Definitions::debugConfig.modedebug)) {
    ...
    return (whiteToMove ? +1 : -1 ) * ( lazy ? Analyse().RunLazy(p) : Analyse().Run(p,taperedCoeff));
}
  ...
  if (Definitions::ttConfig.do_transpositionTableSearch) {
      ...
      if (Definitions::ttConfig.do_transpositionTableSearch && ttok 
          ...
          if (std::find(moves.begin(), moves.end(), ttmove) != moves.end()) {
              ...
              ///@todo this checks looks quite shitty to me !
