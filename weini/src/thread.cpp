...
void SearcherBase::SetData(Position & p, DepthType depth, Line pv, bool  allowNullMove, bool display, bool displayDebug) {
    _data.p             = p;
    _data.depth         = depth;
    _data.pv            = pv;
    _data.allowNullMove = allowNullMove;
    _data.display       = display;
    _data.displayDebug  = displayDebug;
}
...
SearcherBase::SearcherBase(size_t n)
    : _index(n),
      ...
      _data(Position("start"),0,true,true,false),
      _isASync(false){
