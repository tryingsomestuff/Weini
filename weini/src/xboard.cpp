...
std::string XBoard::DisplayInfo(DepthType depth, ScoreType score, float sec, NodeStatType nodes, NodeStatType tbhits, const std::string & pv) {
...
bool XBoard::MakeMove(Move &m, bool displayMove){
    ...
    Position position2 = m.Apply(position,&moveOK);
    if ( ! moveOK ){
...
void XBoard::Loop(){
    ...
    LOG(logCOMGUI) << "feature ping=1 setboard=1 colors=0 usermove=1 memory=0 sigint=0 sigterm=0 otime=0 time=1 nps=0 myname=\"Weini" << Definitions::debugConfig.nameSuffixe << "\"";
