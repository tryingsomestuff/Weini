...
void Analyse::Init(const Position & p){
    const int nbWp = EvalCache::SetEvalCache(_cache.pieceNumber.nbWp,p.NumberOf(Piece::t_Wpawn));
    const int nbWk = EvalCache::SetEvalCache(_cache.pieceNumber.nbWk,p.NumberOf(Piece::t_Wknight));
    const int nbWb = EvalCache::SetEvalCache(_cache.pieceNumber.nbWb,p.NumberOf(Piece::t_Wbishop));
    const int nbWr = EvalCache::SetEvalCache(_cache.pieceNumber.nbWr,p.NumberOf(Piece::t_Wrook));
    const int nbWq = EvalCache::SetEvalCache(_cache.pieceNumber.nbWq,p.NumberOf(Piece::t_Wqueen));
    const int nbBp = EvalCache::SetEvalCache(_cache.pieceNumber.nbBp,p.NumberOf(Piece::t_Bpawn));
    const int nbBk = EvalCache::SetEvalCache(_cache.pieceNumber.nbBk,p.NumberOf(Piece::t_Bknight));
    const int nbBb = EvalCache::SetEvalCache(_cache.pieceNumber.nbBb,p.NumberOf(Piece::t_Bbishop));
    const int nbBr = EvalCache::SetEvalCache(_cache.pieceNumber.nbBr,p.NumberOf(Piece::t_Brook));
    const int nbBq = EvalCache::SetEvalCache(_cache.pieceNumber.nbBq,p.NumberOf(Piece::t_Bqueen));

    EvalCache::SetEvalCache(_cache.pieceNumber.nbW,nbWk + nbWb + nbWr + nbWq);
    EvalCache::SetEvalCache(_cache.pieceNumber.nbB,nbBk + nbBb + nbBr + nbBq);

    EvalCache::SetEvalCache(_cache.pieceNumber.nbWm,nbWk + nbWb);
    EvalCache::SetEvalCache(_cache.pieceNumber.nbBm,nbBk + nbBb);

    EvalCache::SetEvalCache(_cache.kingEval.wks,p.WhiteKingSquare());
    EvalCache::SetEvalCache(_cache.kingEval.bks,p.BlackKingSquare());
}
