...
ScoreType Analyse::EvalMaterial(const Position & p, float taperedCoeff, bool doTradeOffBonus) {
  ...
  // just material
  ScoreType count = Util::Count(p);

  // winning side want to exchange piece
  if (doTradeOffBonus) {
      const int nbWp = GetEvalCache(_cache.pieceNumber.nbWp,p.NumberOf(Piece::t_Wpawn));
      const int nbWk = GetEvalCache(_cache.pieceNumber.nbWk,p.NumberOf(Piece::t_Wknight));
      const int nbWb = GetEvalCache(_cache.pieceNumber.nbWb,p.NumberOf(Piece::t_Wbishop));
      const int nbWr = GetEvalCache(_cache.pieceNumber.nbWr,p.NumberOf(Piece::t_Wrook));
      const int nbWq = GetEvalCache(_cache.pieceNumber.nbWq,p.NumberOf(Piece::t_Wqueen));
      const int nbBp = GetEvalCache(_cache.pieceNumber.nbBp,p.NumberOf(Piece::t_Bpawn));
      const int nbBk = GetEvalCache(_cache.pieceNumber.nbBk,p.NumberOf(Piece::t_Bknight));
      const int nbBb = GetEvalCache(_cache.pieceNumber.nbBb,p.NumberOf(Piece::t_Bbishop));
      const int nbBr = GetEvalCache(_cache.pieceNumber.nbBr,p.NumberOf(Piece::t_Brook));
      const int nbBq = GetEvalCache(_cache.pieceNumber.nbBq,p.NumberOf(Piece::t_Bqueen));

      const int nbW  = GetEvalCache(_cache.pieceNumber.nbW,nbWk + nbWb + nbWr + nbWq);
      const int nbB  = GetEvalCache(_cache.pieceNumber.nbB,nbBk + nbBb + nbBr + nbBq);

      const int nbWm = GetEvalCache(_cache.pieceNumber.nbWm,nbWk + nbWb);
      const int nbBm = GetEvalCache(_cache.pieceNumber.nbBm,nbBk + nbBb);

      //const int nbWM = GetNumber(_cache.pieceNumber.nbWM,nbWr + nbWq);
      //const int nbBM = GetNumber(_cache.pieceNumber.nbBM,nbBr + nbBq);

      // the side with advantage wants to trade piece and keep pawn
      // the side without wants to trade pawn and keep piece
      if ( nbWq == nbBq && nbWm == nbBm){
      ...
      if (count > Definitions::evalBonus.tradeOffLimit) { // if white is ahead
         if ( nbW < nbB ){ // ahead with less piece
             count /= 3;
         }
      }
      else if (count < -Definitions::evalBonus.tradeOffLimit) { // if black is ahead
         if ( nbB < nbW ){ // ahead with less piece
             count /= 3;
         }
      }
...
ScoreType Analyse::EvalPiecePair(const Position & p, float taperedCoeff)  {
  ...
  ScoreType ret = 0;
  ret += GetEvalCache(_cache.pieceNumber.nbWr,p.NumberOf(Piece::t_Wrook))   * Definitions::evalBonus.adjRook  [nbWp];
  ret -= GetEvalCache(_cache.pieceNumber.nbBr,p.NumberOf(Piece::t_Brook))   * Definitions::evalBonus.adjRook  [nbBp];
  ret += GetEvalCache(_cache.pieceNumber.nbWk,p.NumberOf(Piece::t_Wknight)) * Definitions::evalBonus.adjKnight[nbWp];
  ret -= GetEvalCache(_cache.pieceNumber.nbBk,p.NumberOf(Piece::t_Bknight)) * Definitions::evalBonus.adjKnight[nbBp];

  // bishop pair
  ret += ( (GetEvalCache(_cache.pieceNumber.nbWb,p.NumberOf(Piece::t_Wbishop)) > 1 ? Definitions::evalBonus.bishopPairBonus : 0)
          -(GetEvalCache(_cache.pieceNumber.nbBb,p.NumberOf(Piece::t_Bbishop)) > 1 ? Definitions::evalBonus.bishopPairBonus : 0) );
  // knight pair
  ret += ( (GetEvalCache(_cache.pieceNumber.nbWk,p.NumberOf(Piece::t_Wknight)) > 1 ? Definitions::evalBonus.knightPairMalus : 0)
          -(GetEvalCache(_cache.pieceNumber.nbBk,p.NumberOf(Piece::t_Bknight)) > 1 ? Definitions::evalBonus.knightPairMalus : 0) );
  // rook pair
  ret += ( (GetEvalCache(_cache.pieceNumber.nbWr,p.NumberOf(Piece::t_Wrook)) > 1 ? Definitions::evalBonus.rookPairMalus   : 0)
          -(GetEvalCache(_cache.pieceNumber.nbBr,p.NumberOf(Piece::t_Brook)) > 1 ? Definitions::evalBonus.rookPairMalus   : 0) );
...
ScoreType Analyse::EvalPawnShield(const Position & p, float taperedCoeff)  {
  ...
  float ret = 0;
  const Square::LightSquare wks = GetEvalCache(_cache.kingEval.wks,p.WhiteKingSquare());
...
ScoreType Analyse::EvalConnectedRooks(const Position & p, float taperedCoeff)  {
  ...
  ScoreType wb = 0;
  if ( GetEvalCache(_cache.pieceNumber.nbWr,p.NumberOf(Piece::t_Wrook)) == 2 ){
  ...
  ScoreType bb = 0;
  if ( GetEvalCache(_cache.pieceNumber.nbBr,p.NumberOf(Piece::t_Brook)) == 2 ){
...
ScoreType Analyse::EvalOptimDriver(const Position & p, float taperedCoeff, bool display){
  ...
  for(Square::RankFile rank = 0 ; rank < 8 ; ++rank){
    for (Square::RankFile file = 0 ; file < 8 ; ++file){
      ...
      if (Definitions::evalConfig.evalWithBadBishop) {
          const bool isWS = ((BitBoards::whiteSquare & (1ull<<k)) !=0);
  ...
  if ( !p.IsEndGame() && Definitions::evalConfig.evalWithPawnStorm) {
      ...
      if (wkr < 2 && wkf != 4 && GetEvalCache(_cache.pieceNumber.nbBq,p.NumberOf(Piece::t_Bqueen)) != 0 )  {
      ...
      if (bkr > 5 && bkf != 4 && GetEvalCache(_cache.pieceNumber.nbWq,p.NumberOf(Piece::t_Wqueen)) != 0 ) {
  ...
  if (Definitions::evalConfig.evalWithPawnStructure ){
     ...
     const int nbW = GetEvalCache(_cache.pieceNumber.nbW, GetEvalCache(_cache.pieceNumber.nbWk,p.NumberOf(Piece::t_Wknight))
                                                        + GetEvalCache(_cache.pieceNumber.nbWb,p.NumberOf(Piece::t_Wbishop))
                                                        + GetEvalCache(_cache.pieceNumber.nbWr,p.NumberOf(Piece::t_Wrook))
                                                        + GetEvalCache(_cache.pieceNumber.nbWq,p.NumberOf(Piece::t_Wqueen))  );
     const int nbB = GetEvalCache(_cache.pieceNumber.nbB, GetEvalCache(_cache.pieceNumber.nbBk,p.NumberOf(Piece::t_Bknight))
                                                        + GetEvalCache(_cache.pieceNumber.nbBb,p.NumberOf(Piece::t_Bbishop))
                                                        + GetEvalCache(_cache.pieceNumber.nbBr,p.NumberOf(Piece::t_Brook))
                                                        + GetEvalCache(_cache.pieceNumber.nbBq,p.NumberOf(Piece::t_Bqueen))  );
     if ( nbW != nbB ){
        retPawnStructure1 /= 3;
     }
  }
