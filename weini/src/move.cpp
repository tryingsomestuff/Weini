...
bool Move::Ordering(FastContainer<Move> &moves, Position & p, Searcher & searcher, bool isPV,
                    const DepthType depth, const PlyType ply, const Transposition * ttt, const Line * pv, bool trustedMove){
    ...
    if (! searcher.IsMainThread() && Definitions::smpConfig.shuffleThreadMoves ) {
        ...
        if (/*searcher.IsMainThread() &&*/ ttt && ttt->move != 0){
          OPTIM_STATS(++Stats::ttsorttry;)
          if (Transposition::Sort( *ttt, moves, p)) {
            bestMoveFound = true;
            OPTIM_STATS(++Stats::ttsort;)
          }
        }

        // PV node first !
        if (/*searcher.IsMainThread() &&*/ ! bestMoveFound && pv && depth > 0 ){
          OPTIM_STATS(++Stats::pvrootsorttry;)
          if ( Sort::PVFirst(*pv,0,moves)){
             bestMoveFound = true;
             OPTIM_STATS(++Stats::pvrootsort;)
          }
        }
        ...
        if (/*searcher.IsMainThread() &&*/ ! bestMoveFound && isPV ){
          ...
          if ( Definitions::sortingConfig.do_internaliterativeDeepening && depth >= Definitions::selectivity.IIDMinDepth ){
              SearcherBase::SearchedMove iid = searcher.SearchRoot(-Definitions::scores.infScore,Definitions::scores.infScore,moves,p,depth/2,ply, ///@todo try -2 ?
                                                                   NULL,  // never update pv here
                                                                   TimeMan::eTC_on, // subject to TC ?
                                                                   Searcher::eRedA_on,
                                                                   Searcher::eExtA_off,
                                                                   Move::HashType(0),
                                                                   trustedMove);  // given moves are trusted
             OPTIM_STATS(++Stats::iidsort;)
             if (SearcherBase::GetMove(iid).IsValid() ){
                 if (!Sort::IIDFirst(iid, moves)) {
...
bool Move::Validate(Position & p, bool validateIsCheck, bool checkValidity)/*const*/{
    ...
    if ( _moveType > UtilMove::sm_standard ){
        ...
        else{ // promotion (can be a capture at the same time !)
            isPromotion = true;
        }
    }

    UtilMove::DetectStandardCapture(*this,p); // not ep capture
    ...
    case Piece::t_Bpawn:
        ...
        isOK = UtilMove::ValidatePawn(*this,p); // will also set capture if ep
        break;
    case Piece::t_Wpawn:
...
Position Move::ValidateAndApply(const Position &p, bool * success) {
   ...
   Position p2(p);
    ...
    if( Validate(p2) ) { // THIS IS SLOWWWWWWW but bullet proof
        p2.ApplyMove(*this);
...
Position & Move::ValidateAndApplyInPlace(Position & p, bool *success) {
    bool b = false;
    Position p2 = ValidateAndApply(p, &b);
