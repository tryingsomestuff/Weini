...
bool Move::Ordering(FastContainer<Move> &moves, Position & p, Searcher & searcher, bool isPV,
    ...
    if (! searcher.IsMainThread() && Definitions::smpConfig.shuffleThreadMoves ) {
        ...
        if (/*searcher.IsMainThread() &&*/ ! bestMoveFound && isPV ){
          ...
          if ( Definitions::sortingConfig.do_internaliterativeDeepening && depth >= 5 ){
              SearcherBase::SearchedMove iid = searcher.SearchRoot(-Definitions::scores.infScore,Definitions::scores.infScore,moves,p,depth >= 6?depth/4:1,
