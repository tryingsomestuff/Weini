  ...
  if (/*! p.isEndGame() &&*/
    ...
    if (Definitions::algo.do_razoring &&
        ...
        if (alreadyComputedStandPat + margin <= alpha) {
            ...
            ScoreType val = NegaQuiesce(rAlpha, rAlpha+1, (int)Definitions::selectivity.quies_max, p, color, fromPV, false);
            if (val <= rAlpha) {
    ...
    if ( Definitions::algo.do_probCut &&
      ...
      for (auto it = moves.begin() ; it != moves.end() && probCutCount < Definitions::selectivity.probCutMaxMoves; ++it){
        ...
        if ((*it).ZHash() == ttt.hash && (IsSilent(*it,p) || !SEE(p,*it))){
          continue;
        }
        ...
        ScoreType scorePC = -Negamax(-betaPC, -betaPC + 1,
