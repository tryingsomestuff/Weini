...
namespace {
    ...
    bool ManageExtensionNegamax(DepthType &                 depth,
        ...
        if (Definitions::extensionReductionConfig.do_CheckExtensionAlphaBeta
            && move.IsCheck()) {
            ++depth;
            extended = true;
        ...
        if (Definitions::extensionReductionConfig.do_ReCaptureExtensionAlphaBeta
            && isPV
        ...
        if (Definitions::extensionReductionConfig.do_SingleReplyExtensionAlphaBeta
            && moves.size() == 1) {
            ++depth;
            extended = true;
        ...
        if (Definitions::extensionReductionConfig.do_NearPromotionExtensionAlphaBeta
        ...
        if (Definitions::extensionReductionConfig.do_SingularExtensionAlphaBeta
            && depth >= 8
            && ttt.depth >= depth - 3
...
ScoreType Searcher::Negamax(ScoreType       alpha,
  ...
  // =====================
  if ( Searcher::IsForcedDraw(game, p, analyze.MaterialCache(), 1, Definitions::debugConfig.modedebug)){
  ...
  */
  ...
  if (/*! p.isEndGame() &&*/
    ...
    // <= alpha - margin
    // =====================
    if (Definitions::algo.do_razoring &&
        // alpha > Definitions::previousScore[p.Turn()] - Definitions::lmrtol &&
        depthleft <= Definitions::selectivity.razoringMaxDepth &&
        iterativeDeepeningDepth > Definitions::selectivity.iterativeDeepeningSelectivityMinDepth &&
        // !p.IsEndGame() &&
        // ! p.Info().lastMoveWasCapture() && // no Razoring after a capture
        !Move::IsNearPromotion(p, p.Info().lastMove.to)) { // to not perform razoring just before promotion
        ...
        if (alreadyComputedStandPat + margin <= alpha) {
            ...
            ++Stats::razoringbetacutQtry;
            ScoreType rAlpha = alpha - margin;
            const DepthType qdepth = Definitions::algo.do_quiesce?(int)Definitions::selectivity.quies_max:0;
            ScoreType val = NegaQuiesce(rAlpha, rAlpha+1, qdepth, p, color, fromPV, false);
            if (val <= rAlpha) {
                 ++Stats::razoringbetacutQ;
            ...
            if (nullScore /*- Definitions::nullMoveMargin*/ >= beta) { // too good move
              ...
              if (Definitions::algo.do_verifiedNullMove && R >= Definitions::selectivity.verifiedNullMoveMinDepth) {
                  ...
                  nullScore = (depthleft - R -1 > 0) ?  Negamax(beta-1, beta, depthleft - R - 1, p, color, iterativeDeepeningDepth/*0*/,
                                                          false, NULL, /*0*/ false, false, true)
                                                :  NegaQuiesce(beta-1, beta, qdepth, p, color,
                                                          false, false);
                  if (nullScore /*- Definitions::nullMoveMargin*/ >= beta) { // too good move
    ...
    // ProbCut
    // =====================
    if ( Definitions::algo.do_probCut &&
      ...
      for (auto it = moves.begin() ; it != moves.end() && probCutCount < Definitions::selectivity.probCutMaxMoves; ++it){
        ...
        const DepthType qdepth = Definitions::algo.do_quiesce?(int)Definitions::selectivity.quies_max:0;
        ScoreType scorePC = -NegaQuiesce(-betaPC,-betaPC + 1,qdepth,p2,color,false,false);
    ...
    if (Definitions::algo.do_futilityPruning &&
        ...
        pruningAllowed = (alreadyComputedStandPat + margin <= alpha);
  ...
  for (auto it = moves.begin() ; it != moves.end() ; ++it, ++moveCount){
    ...
    */
    ...
    // =====================
    if ( ! isFirstMove ){
       if ( Definitions::extensionReductionConfig.do_ttmoveCaptureReduction
