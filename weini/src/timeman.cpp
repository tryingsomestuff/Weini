...
int TimeMan::GetNextTimePerMove(const Position & position) {
    ...
    case TCT_classic:
    {
        ...
        remainingMoveUntilNextTC = moveNumberForNextTC - position.Moves() + 1;
        ...
        if (msecRemainingTimeInTC < 0) {
            ...
            msecRemainingTimeInTC = 100; // let's try that ...
        }
        ...
        if (msecPerMove >= msecRemainingTimeInTC) {
        ...
        LOG(logINFO) << "                 mps                        " << mps;
        LOG(logINFO) << "                 current moves              " << position.Moves();
    ...
    case TCT_suddenDeath:
    {
        ...
        remainingMoveUntilNextTC = std::max((int)Definitions::timeControl.minMoveToGo,
                                            (int)Definitions::timeControl.maxMoveToGo - position.Moves());
...
int TimeMan::GetNextMaximalTimePerMove(const Position & position) {
    return std::min(10*msecPerMove, int(0.85*msecRemainingTimeInTC)); // this is a security margin
}
