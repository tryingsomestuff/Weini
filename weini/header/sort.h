    ...
    bool operator()(const Move & m1, const Move & m2){
        ...
        if ( s1 == Definitions::scores.defaultMoveScore){
            if ( t1 == UtilMove::sm_standard || t1 == UtilMove::sm_ep){
                ...
                // check first
                if ( Definitions::sortingConfig.do_sortchessbonus && m1.IsCheck() ){ // but check information is probably no available yet ...
        ...
        if ( s2 == Definitions::scores.defaultMoveScore){
            if ( t2 == UtilMove::sm_standard || t2 == UtilMove::sm_ep){
                ...
                // check first
                if ( Definitions::sortingConfig.do_sortchessbonus && m2.IsCheck() ){
