...
struct SortMovesFunctor {
    ...
    const Position & _p;
    const Searcher & _s;
    const MiniMove & _m; // last move
    float taperedCoeff;
    bool whiteToPlay;
    SortMovesFunctor(NSColor::eColor c, const Position & p, const Searcher & searcher) : color(c), _p(p), _s(searcher), _m(_s.game.Info(_p.CurrentPly()).lastMove) {
        taperedCoeff = (_p.GamePhasePercent() - Position::endGameGamePhasePercent) / (100.f - Position::endGameGamePhasePercent);
        whiteToPlay = _p.WhiteToPlay();
    }
    bool operator()(const Move & m1, const Move & m2){
        ...
        if ( s1 == Definitions::scores.defaultMoveScore){
            if ( t1 == UtilMove::sm_standard || t1 == UtilMove::sm_ep){
                ...
                if (Definitions::sortingConfig.do_captureLastMovedBonus && m1.IsCapture() && m1.To() == _m.to) {
                ...
                if (Definitions::sortingConfig.do_counterHeuristic && !m1.IsCapture()
                    ...
                    if (_s.counterMove[_m.from][_m.to] == m1.ZHash()) {
        ...
        if ( s2 == Definitions::scores.defaultMoveScore){
            if ( t2 == UtilMove::sm_standard || t2 == UtilMove::sm_ep){
                ...
                // Capture last moved piece bonus
                if (Definitions::sortingConfig.do_captureLastMovedBonus && m2.IsCapture() && m2.To() == _m.to) {
                ...
                if (Definitions::sortingConfig.do_counterHeuristic && !m2.IsCapture()
                    ...
                    if (_s.counterMove[_m.from][_m.to] == m2.ZHash()) {
...
struct SortQMovesFunctor {
    ...
    const Searcher & _s;
    const MiniMove & _m;
    SortQMovesFunctor(NSColor::eColor c, const Position & p, const Searcher & searcher) : color(c), _p(p), _s(searcher), _m(_s.game.Info(_p.CurrentPly()).lastMove) {
    }
    bool operator()(const Move & m1, const Move & m2){
        //LOG(logINFO) << m1.Show() << " vs " << m2.Show();
...
static inline
void SortQMoves(FastContainer<Move> & moves,const Position & p, const Searcher & searcher){
    int k = 0;
    for(auto it = moves.begin() ; it != moves.end() ; ++it){
        if ( (*it).ZHash() == 0 ) {
    ...
    moves.sort(SortQMovesFunctor(p.Turn(), p, searcher));
    ///@todo use sortPhase ?
}
