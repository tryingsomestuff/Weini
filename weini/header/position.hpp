...
inline
Position::AdditionalInfo::AdditionalInfo() :
    enPassant(Square::Invalide),
...
inline
Position::AdditionalInfo::AdditionalInfo(const AdditionalInfo & i) :
    white(i.white),
    black(i.black),
    enPassant(i.enPassant),
    lastMove(i.lastMove),
    lastCapturedPiece(i.lastCapturedPiece),
    isInCheck(i.isInCheck),
    halfMovesSinceLastPawnMoveOrCapture(i.halfMovesSinceLastPawnMoveOrCapture),
    movesCount(i.movesCount),
    currentply(i.currentply),
    gamePhase(i.gamePhase),
    zhash(i.zhash),
    zhashpawn(i.zhashpawn)
{
    //fen[0] = '\0';
    //std::strncpy(fen,i.fen,MAX_FEN_SIZE);
}
...
inline
void Position::AdditionalInfo::operator=(const AdditionalInfo & i) {
    white = i.white;
    black = i.black;
    enPassant = i.enPassant;
    lastMove = i.lastMove;
...
inline
const Position::AdditionalInfo & Position::Info()const { return _info; }

inline
Position::AdditionalInfo & Position::InfoRef() { return _info; }
...
Piece::eType & Position::_RGet(Square::LightSquare k, bool invalidateAlsoHash) {
    invalidateAlsoHash ? Invalidate() : InvalidateFen(); // you should use get if not using the reference for modification !!
#ifdef DEBUG_POSITION_GET
    assert(k >= 0);
    assert(k <= 63);
#endif
...
Piece::eType & Position::_RGet(Square::RankFile file, Square::RankFile rank, bool invalidateAlsoHash) {
    invalidateAlsoHash ? Invalidate() : InvalidateFen();
#ifdef DEBUG_POSITION_GET
    assert(file >= 0);
    assert(file <= 7);
    assert(rank >= 0);
...
void Position::SetPiece(Square::LightSquare k, Piece::eType t) {
    // update available
    const Piece::eType t1 = Get(k);

    _SetPieceB(k, t, t1);

    // update hashes
    _XORHash(Util::Zobrist::zTable[(int)k][TypeToInt(t1)]);
    _XORHash(Util::Zobrist::zTable[(int)k][TypeToInt(t)]);
    if (Piece::IsPawn(t1)) {
        _XORHashPawn(Util::Zobrist::zTable[(int)k][TypeToInt(t1)]);
    }
    if (Piece::IsPawn(t)) {
        _XORHashPawn(Util::Zobrist::zTable[(int)k][TypeToInt(t)]);
    }
...
inline void Position::InvalidateFen() {
    //_info.fen[0] = '\0';
}
...
inline bool Position::IsInCheck()const { return _info.isInCheck; }

inline int Position::HalfMoves50Rule()const { return _info.halfMovesSinceLastPawnMoveOrCapture; }
inline int Position::Moves()const { return _info.movesCount; }
inline int Position::CurrentPly()const { return _info.currentply; }
