...
public:
	...
	struct Data {
        inline
        Data(const Position & pp, DepthType d, eVerbosity verbosity)
			: p(pp), depth(d), verbosity(verbosity),searchDone(false) {}
		Position     p;
        DepthType    depth;
		Line         pv;
    ...
    void SetData(Position & p, DepthType depth, Line pv, eVerbosity verbosity);
