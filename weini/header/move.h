...
class Move {
  ...
  friend bool UtilMove::ValidateIsCheck        (const Move & m, const Position & p);
...
public:
  ...
  static bool Ordering(FastContainer<Move> & moves, Position & p, Searcher &searcher, bool isPV,
                       const DepthType DepthType, const PlyType ply, const Transposition * ttt, const Line * pv, bool trustedMove = false);
