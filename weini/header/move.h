...
class Move {
  ...
  friend bool UtilMove::CheckForCapture  (const Move & m, const Position & p);
...
public:
  ...
  */
  ...
  Position Apply(const Position & p, bool *success = NULL);

  // This one is slow, do not use it deep inside the search
  Position & ApplyInPlace(Position & p, bool *success = NULL);
