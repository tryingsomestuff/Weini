...
cutechess_cli_path = '/media/BK1/www/p/chessEngine/weini/cutechess/cutechess-cli/cutechess-cli.sh'

# The engine whose parameters will be optimized
engine = 'name=Weini cmd=/media/BK1/www/p/chessEngine/weini/bin/weini arg=-xboard dir=/media/BK1/www/p/chessEngine/weini/bin/'
...
engine_param_cmd = 'setoption {name} {value}'

# A pool of opponents for the engine. The opponent will be
# chosen based on the seed sent by CLOP.
opponents = [
    'name=fairymax cmd=/usr/games/fairymax'
]

# Additional cutechess-cli options, eg. time control and opening book
options = '-each proto=xboard tc=80/1 -draw movenumber=80 movecount=5 score=5 -resign movecount=5 score=500 -openings file=/media/BK1/www/p/chessEngine/weini/Book/AH-150.pgn format=pgn order=random plies=12'
